/**
 * NightMoodSelector — Pre-session intent setter.
 *
 * Appears once per evening (after 6pm) before the user starts scouting.
 * Sets their "night mood" which influences how their bolt taps are weighted
 * and personalizes the Insider feed. NOT a second rating — purely context.
 *
 * Unified with the bolt system: mood × tap velocity = individual resonance signal.
 */
...
export type NightMood = 'easy_flow' | 'high_energy' | 'mixed_scene' | 'low_key';
...
interface NightMoodSelectorProps {
  visible: boolean;
  onSelect: (mood: NightMood) => void;
  onSkip: () => void;
}

// ─── Component ────────────────────────────────────────────────────────────────

export default function NightMoodSelector({ visible, onSelect, onSkip }: NightMoodSelectorProps) {
  ...
  const handleSelect = (mood: NightMood, idx: number) => {
        ...
        <BlurView intensity={50} tint="dark" style={styles.blurFill}>
          <LinearGradient colors={['rgba(16,10,26,0.97)', 'rgba(8,6,16,0.99)']} style={styles.gradient}>
            ...
            <View style={styles.header}>
              ...
              <View style={{ flex: 1 }}>
                <Text style={styles.title}>Set Your Vibe Intent</Text>
